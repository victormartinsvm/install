...
get_link_git() {
  ...
  print_banner
  printf "${WHITE} 💻 Insira o link do GITHUB do seu Zaplay que deseja instalar:${GRAY_LIGHT}"
  printf "\n\n"
  read -p "> " link_git
}
...
get_max_whats() {
  ...
  print_banner
  printf "${WHITE} 💻 Informe a Qtde de Conexões que ${instancia_add} poderá cadastrar:${GRAY_LIGHT}"
  printf "\n\n"
  read -p "> " max_whats
}
...
get_max_user() {
  ...
  print_banner
  printf "${WHITE} 💻 Informe a Qtde de Usuarios que a ${instancia_add} poderá cadastrar:${GRAY_LIGHT}"
  printf "\n\n"
  read -p "> " max_user
}
...
get_redis_port() {
  ...
  print_banner
  printf "${WHITE} 💻 Digite a porta do REDIS/AGENDAMENTO de MSG para a ${instancia_add}; Ex: 5000 A 5999 ${GRAY_LIGHT}"
  printf "\n\n"
  read -p "> " redis_port
}
...
inquiry_options() {
  ...
  print_banner
  printf "${WHITE} 💻 Bem vindo(a) ao Gerenciador Zaplay, selecione abaixo a proxima ação!${GRAY_LIGHT}"
  printf "\n\n"
  printf "   [0] Instalar Zaplay\n"
  printf "   [1] Atualizar Zaplay\n"
  printf "   [2] Deletar Zaplay\n"
  printf "   [3] Bloquear Zaplay\n"
  printf "   [4] Desbloquear Zaplay\n"
  printf "   [5] Alter. dominio Zaplay\n"
  printf "\n"
  read -p "> " option
